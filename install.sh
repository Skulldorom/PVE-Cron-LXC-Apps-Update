...
write_config() {
  # Write current settings to the config file. Cron now invokes the worker
  # directly; the worker reads this config with a constrained parser (no
  # shell `source`), so arbitrary commands cannot be executed from config.
...
CFGEOF
  ...
  # Retain the legacy wrapper for backward compatibility with any cron entry
  # that still references it, but the wrapper simply delegates to the worker.
  cat > "$WRAPPER_SCRIPT" <<WREOF
#!/bin/bash
# Community Apps Update — legacy cron wrapper (compatibility shim)
# New installs point cron directly at the worker; this shim only exists so
# pre-existing cron entries keep working unchanged.
set -uo pipefail
exec "${LOCAL_SCRIPT}"
WREOF
...
read_config() {
  # Parse the config file with a constrained key/value reader (no shell source)
  # so config files cannot execute arbitrary commands. Sets CONTAINER_IDS,
  # BACKUP_STORAGE, BACKUP, NOTIFY, DRY_RUN, AUTO_REBOOT, UPSTREAM_REFRESH,
  # ALLOW_CACHED_UPSTREAM, HEALTHCHECK_URL in the caller's scope. The schedule
  # is read from crontab via get_schedule(), not stored in the config.
  local file="$CONFIG_FILE"
  [ -f "$file" ] || file="$OLD_CONFIG_FILE"
  [ -f "$file" ] || return 1
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    case "$key" in
      CONTAINERS|CONTAINER_IDS) CONTAINER_IDS="$value" ;;
      BACKUP_STORAGE) BACKUP_STORAGE="$value" ;;
      BACKUP) BACKUP="$value" ;;
      NOTIFY) NOTIFY="$value" ;;
      DRY_RUN) DRY_RUN="$value" ;;
      AUTO_REBOOT) AUTO_REBOOT="$value" ;;
      UPSTREAM_REFRESH) UPSTREAM_REFRESH="$value" ;;
      ALLOW_CACHED_UPSTREAM) ALLOW_CACHED_UPSTREAM="$value" ;;
...
migrate_legacy_config() {
  # Convert the previous /etc/update-community-apps/config (wrapper + source
  # model) into the new /etc/update-community-apps.conf, preserving settings.
  ...
  if [ -n "$entry" ]; then
    ...
    remove_cron || true
    (crontab -l -u root 2>/dev/null || true; echo "${old_sched} ${LOCAL_SCRIPT} >>${LOG_FILE} 2>&1") | crontab -u root -
    msg_ok "Cron entry updated to invoke ${LOCAL_SCRIPT} directly (schedule preserved)."
  fi

  msg_ok "Migration complete. Legacy config kept at ${OLD_CONFIG_FILE}."
}
...
edit_config() {
  ...
  read -r cur_min cur_hour cur_day cur_month cur_dow <<< "$(get_schedule)"
...
Apply changes?" 16 65; then
  ...
  remove_cron || true
  local cron_entry
  cron_entry="${cur_min} ${cur_hour} ${cur_day} ${cur_month} ${cur_dow} ${LOCAL_SCRIPT} >>${LOG_FILE} 2>&1"
...
show_status() {
  ...
  echo ""
  echo -e "  ${YELLOW}─── Upstream Cache ──────────────────────────────────────────${NC}"
  local cache_file="/usr/local/lib/update-community-apps/update-apps.sh"
  local cache_meta="/usr/local/lib/update-community-apps/update-apps.meta"
  if [ -f "$cache_file" ]; then
    local cache_hash cache_age cache_src
    cache_hash=$(sha256sum "$cache_file" | awk '{print $1}')
    cache_src=$(grep -E '^source_url=' "$cache_meta" 2>/dev/null | cut -d= -f2-)
    cache_age=$(grep -E '^refreshed_at=' "$cache_meta" 2>/dev/null | cut -d= -f2-)
    echo -e "  ${GREEN}Present:${NC} yes"
    echo -e "      Source: ${cache_src:-unknown}"
    echo -e "      SHA256: ${cache_hash}"
    echo -e "      Last refreshed: ${cache_age:-unknown}"
  else
    echo -e "  ${YELLOW}Present:${NC} no (will be created on first run)"
  fi
  echo ""
  read -rp "Press Enter to continue..."
}
...
refresh_upstream_cache() {
  ...
  msg_info "Refreshing cached upstream update-apps.sh..."
  if bash "$LOCAL_SCRIPT" --refresh-upstream-cache; then
    msg_ok "Upstream cache refreshed."
  else
    msg_error "Upstream cache refresh failed; existing cache (if any) left untouched."
  fi
  echo ""
  read -rp "Press Enter to continue..."
}
...
run_now() {
  ...
  echo ""
  bash "$LOCAL_SCRIPT" 2>&1 | tee -a "$LOG_FILE"
  echo ""
...
dry_run() {
  ...
  echo ""
  bash "$LOCAL_SCRIPT" --dry-run 2>&1 | tee -a "$LOG_FILE"
  echo ""
...
# ── Remove ───────────────────────────────────────────────────────────────────
remove_all() {
  local keep_config=no
  if [ -f "$CONFIG_FILE" ] || [ -f "$OLD_CONFIG_FILE" ]; then
    if whiptail --backtitle "Community Apps Update" --title "Remove" \
      --yesno "Keep configuration files?\n\nSelect 'Yes' to preserve ${CONFIG_FILE}\nso a future reinstall retains your settings." 11 70; then
      keep_config=yes
    fi
  fi

  if ! whiptail --backtitle "Community Apps Update" --title "Remove" \
    --yesno "Remove the cron schedule and local script?\n\nThis will delete:\n  • Cron entry\n  • ${LOCAL_SCRIPT}\n  • Cached upstream update-apps.sh\n\nLog files will be kept." 13 65; then
    return
  fi
  ...
  # Cached upstream script and metadata
  rm -f /usr/local/lib/update-community-apps/update-apps.sh \
        /usr/local/lib/update-community-apps/update-apps.meta 2>/dev/null || true
  msg_ok "Removed cached upstream update-apps.sh"
  # Last-run status file
  rm -f /var/log/update-community-apps-last-status 2>/dev/null || true
  msg_ok "Removed last-run status file"

  if [ "$keep_config" = yes ]; then
    msg_info "Configuration kept."
  else
    if [ -f "$CONFIG_FILE" ]; then rm -f "$CONFIG_FILE"; msg_ok "Removed ${CONFIG_FILE}"; fi
    if [ -f "$OLD_CONFIG_FILE" ]; then rm -f "$OLD_CONFIG_FILE"; msg_ok "Removed ${OLD_CONFIG_FILE}"; fi
  fi
  msg_info "Log files kept in /var/log (remove manually if desired)."
  echo ""
  read -rp "Press Enter to continue..."
}
...
update_script() {
  ...
  # Re-run write_config with values from the existing config file to pick up
  # any config-template changes without altering user settings.
  if [ -f "$CONFIG_FILE" ] || [ -f "$OLD_CONFIG_FILE" ]; then
    msg_info "Regenerating wrapper script..."
    if ! read_config; then
      msg_error "Failed to read config; wrapper not regenerated."
      read -rp "Press Enter to continue..."
      return
    fi
...
\nProceed with installation?" 19 65 || {
  ...
  # Cron entry calls the worker directly, which reads the persistent config.
  local cron_entry
  cron_entry="${cron_min} ${cron_hour} ${cron_day} ${cron_month} ${cron_dow} ${LOCAL_SCRIPT} >>${LOG_FILE} 2>&1"
...
main_menu() {
  while true; do
    ...
    choice=$(whiptail --backtitle "Community Apps Update" --title "Main Menu" \
      --menu "Select an option:" 21 65 11 \
      ...
      "Refresh Cache" "Refresh cached upstream update-apps.sh" \
    ...
    case "$choice" in
      ...
      "Dry Run") dry_run ;;
      "Run Now") run_now ;;
      "Refresh Cache") refresh_upstream_cache ;;
      "Logs")    logs_menu ;;
      "View")    view_config ;;
      "Exit")    clear; exit 0 ;;
    esac
